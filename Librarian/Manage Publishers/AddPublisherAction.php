...
<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        $sql = "INSERT INTO publisher (Publisher_Name, Contact_Num, Contact_Mail, Address	)
        VALUES (?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);

        $stmt->bind_param("ssss", $name, $contactNumber, $email, $address);
        if ($stmt->execute()) {
            echo "Publisher has been added successfully.";
        } else {
            echo "Error adding the publisher " . $conn->error;
        }
