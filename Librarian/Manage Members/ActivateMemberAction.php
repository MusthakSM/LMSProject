...
<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        $selectSql = "SELECT userName, Status FROM member WHERE Member_Id = ?";
        ...
        if ($row && $row["userName"] === $mem_username) {
            ...
            // check if the provided user is activ or not
            if($row && $row["Status"] === "inactive") {
                // Prepare and execute the SQL UPDATE statement
                $sql = "UPDATE member SET Status = 'active' WHERE Member_Id = ?";
                ...
                if ($stmt->execute()) {
                    ...
                    echo "Error in activation.. " . $conn->error;
                }

                $stmt->close();

            } else{
                // member is active already..
                echo "Member is active already...";
            }
