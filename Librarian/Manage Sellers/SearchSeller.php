        ...
        <div class="container-fluid d-flex flex-column min-vh-100" style="padding-right: 0; padding-left: 0;">
            ...
            <?php
                ...
                if($_SERVER["REQUEST_METHOD"] === "POST")
                {
                    ...
                    // Check if the first letter of the searchKey is 'S' and the second letter is a number.. then a search by the id
                    if (substr($searchKey, 0, 1) === 'S' && is_numeric(substr($searchKey, 1, 1))) {
                        // Search by seller Id
                        $sq1 = "SELECT Seller_Id, Seller_Name, Contact_Num, Contact_Mail, Address FROM SELLER WHERE Seller_Id = ?";
                        ...
                        if ($selectStmt) {
                            ...
                            if ($results->num_rows === 0) {
                                ...
                                while ($row = $results->fetch_assoc()) {
                                    echo '<tr>';
                                    echo '<td class="text-center">' . $row['Seller_Id'] . '</td>';
                                    echo '<td class="text-center">' . $row['Seller_Name'] . '</td>';
                        ...
                        // Search by name
                        $sq1 = "SELECT Seller_Id, Seller_Name, Contact_Num, Contact_Mail, Address FROM SELLER WHERE Seller_Name = ?";
                        $selectStmt = $conn->prepare($sq1);
                        $selectStmt->bind_param("s", $searchKey);
